...
impl MyApp {
   ...
   pub fn new(cc: &eframe::CreationContext) -> Self {
      ...
      let mut app: MyApp = Default::default(); // app.plt_ctx.hash_map is empty
   ...
   fn plt_data(&mut self, ui: &mut Ui, data: &Array2<f64>) {
      for list in self.show_chans.iter() {
         for chn in list {
            ...
            let mut plt: Plot = self.plt_ctx.plot(format!("p{chn}"), self.plt_size, self.samples, self.yaxis_on);
            if self.adjust_y {
